...
一个个人管理面板，用来管理任务、倒计时事件、定时检查、电费监控、提醒、每日评分和 AI 建议。
...
- 当前任务分：根据普通任务和定时检查中的逾期事项计算 0-100 分，并用颜色显示健康度。
- 倒计时表：新增、查看、编辑、删除未来事件，显示距离发生还有多久。
- Microsoft To Do：连接账号、拉取任务、推送本地任务、本地完成/恢复时尝试同步。
- 定时检查：周期事项配置、打卡、跳过、自动计算下次到期、逾期提醒、睡眠手动记录。
- 电费监控：录入当前或历史电费读数，记录充值情况，估算近期日耗电，在低于 15 度或即将低于阈值时提示。
